...
\begin{abstract}
克尔效应引起的非线性损伤对相干光通信系统的可达数据率和传输距离构成了根本性限制。尽管数字反向传播（DBP）技术可以有效缓解这些效应，但其过高的计算复杂度使得该方案难以在功耗受限的收发机中实际部署。本文研究了一种轻量级多层感知器（MLP）架构，用于在标准单模光纤800~km单信道16-QAM系统中，接续电子色散补偿（EDC）之后进行非线性补偿（NLC）。所提出的补偿器采用紧凑的带记忆MLP结构，仅包含3,346个可训练参数，在$-4$~dBm至$+4$~dBm的入纤功率范围内，实现了从8.50~dB到38.56~dB的$Q$因子增益。误码率（BER）统计结果表明，在EDC单独处理已超出硬判决前向纠错（HD-FEC）门限的高入纤功率下，MLP-NLC可将误码抑制至可测量极限以下。计算复杂度分析表明，该MLP每符号仅需$3.23 \times 10^3$次实数乘法运算，约为标准DBP运算量的0.09\%。上述结果表明，带记忆的浅层前馈神经网络能够以远低于物理模型驱动DBP的计算代价，实现显著的非线性损伤抑制，是面向低功耗相干接收机的有力候选方案。
\end{abstract}
...
在接收端，电子色散补偿（EDC）通过一个简单的频域全通滤波器即可高效地反转线性色散信道。然而，EDC对光纤非线性无能为力。随着入纤功率增大，非线性损伤逐渐加剧并最终成为信号劣化的主导因素，限制了系统可达到的$Q$因子和误码率。

数字反向传播（DBP）~\cite{ip2008}作为一种基于物理模型的补偿技术已被广泛研究。该方法将光纤划分为多个短段，交替施加色散和非线性相位算子，以数值方式逆向求解NLSE。尽管其在概念上简洁优雅，但标准DBP每个光纤跨段需要数百个分步傅里叶法（SSFM）步骤，每一步都涉及计算开销极大的快速傅里叶变换（FFT），导致计算负荷通常比EDC高出3--5个数量级。降步DBP变体以精度换取复杂度，但当每跨段步数降至约10步以下时，补偿性能将显著下降。
...
本文在单信道16-QAM、$10 \times 80$~km标准单模光纤（SSMF）传输场景下，实现并评估了一种复杂度感知的MLP非线性补偿器。主要贡献包括：

\begin{enumerate}
    \item 一个从第一性原理SSFM光纤建模到MLP训练与性能评估的完整仿真流水线，所有代码均已开源。
    \item 系统性的功率扫描分析（$-4$至$+4$~dBm），量化了入纤功率、非线性强度与MLP补偿增益之间的定量关系。
    \item 详细的复杂度对比分析，表明所提出的MLP每符号所需实数运算量仅为标准DBP的0.09\%。
    \item 基于理论EVM估计和直接Gray映射16-QAM比特计数的误码率分析。
\end{enumerate}
...
\caption{16-QAM星座图概览：五种入纤功率下的EDC输出（上行）与原始发射符号（下行），展示了随功率增大而加剧的非线性失真。}
\label{fig:overview}
\end{figure}
...
记忆机制至关重要：无记忆的单符号MLP（$M = 0$）无法捕获色散非线性在时域上的相关性。通过引入相邻符号，网络能够学习局部符号间非线性相互作用模式，其机理与DBP处理整个波形的思路类似。然而，与DBP需要对整个信号执行完整的FFT运算不同，MLP以固定大小的窗口逐符号操作，天然支持并行化的批量推理。
...
表~\ref{tab:evm_results}汇总了所有入纤功率下经MLP补偿后的EVM和$Q$因子性能。在最高功率下，EVM降低比超过98\%，确认了MLP成功学习了确定性的非线性映射关系。经MLP补偿后的残余EVM（$+4$~dBm时为0.0041）已接近测量噪声本底。

\begin{table}[h]
\centering
\caption{EVM与$Q$因子结果（测试集）}
...
\caption{$Q$因子随入纤功率变化曲线：EDC（蓝色方块）vs.~MLP-NLC（橙色圆圈）。标注显示了各功率级别的$Q$增益。}
\label{fig:qfactor}
\end{figure}
...
\caption{BER随入纤功率变化曲线：EDC（蓝色）vs.~MLP-NLC（橙色）。红色虚线标注HD-FEC门限（$1.5 \times 10^{-2}$）。标注显示了各功率级别的BER降低倍数。}
\label{fig:ber}
\end{figure}
...
MLP每符号仅需$3.23 \times 10^3$次实数乘法运算，约为标准DBP的0.094\%、降阶DBP的4.7\%。在推理延迟方面，每符号处理时间约为85.6~ns，对应单个CPU核（标准x86处理器实测）约11.7~Msymbols/s的吞吐率。在32~GBaud的目标符号率下，这意味着约需2,735个CPU核来满足实时处理要求——若采用专为小型矩阵-向量乘法优化的专用神经处理单元（NPU）或FPGA实现，该数值可降低数个数量级。
...
\caption{MLP-NLC带来的$Q$因子增益随入纤功率变化图（柱状图，左轴），叠加了补偿前EDC EVM曲线（红线，右轴）。增益随非线性强度增大而增加，证实了MLP主要针对确定性克尔损伤进行补偿。}
\label{fig:qgain}
\end{figure}

\subsection{讨论}

仿真结果清晰地揭示了不同工作区间之间的层次结构。在ASE噪声占主导的低功率区间（$-4$~dBm），MLP提供了$+8.50$~dB的中等增益——受限于ASE噪声本质上的随机性，这一提升虽有用但有限。在非线主导的高功率区间（$+4$~dBm），MLP实现了$38.56$~dB的显著$Q$因子改善，从本已无法使用的星座图中恢复出近乎完美的信号。这一行为在定性层面是符合预期的：确定性的非线性相移可以从符号模式中学习，而热噪声和量子噪声则无法被任何确定性补偿器消除。

采用$M = 2$个记忆抽头的滑动窗口方法在上下文范围与模型复杂度之间取得了良好的平衡。在10维输入特征下，第一个隐藏层64个神经元配备704个可训练参数，形成一个适度过完备的表示，能够在不过拟合的前提下捕获符号间的非线性相互作用。在上述仿真色散水平下，非线性信道记忆主要集中在邻近的若干个符号内，更宽或更深的网络架构将增加参数量而收益可能递减。

一个值得注意的观察结果是，尽管EDC EVM在不同功率间的变化幅度超过2倍，但MLP补偿后的EVM在所有功率级别下均保持近似恒定（约0.004--0.063）。补偿效果在高功率下最为显著，此时非线性与ASE的比值最大，由此确认残余EVM下限主要由ASE噪声分量决定，而任何确定性补偿器均无法消除该分量。

\section{结论}
\label{sec:conclusion}

本文提出了一个完整的MLP基光纤非线性补偿仿真与评估框架，针对单信道16-QAM相干系统进行了全面分析。主要结论如下：
...
未来工作可从以下几个方向展开：（a）训练一个跨功率级别感知的单一统一MLP模型；（b）将本方法扩展至双偏振和WDM场景，此时跨信道非线性将变得显著；（c）探索更先进的网络架构，如基于注意力机制的模型或Kolmogorov-Arnold网络~\cite{gdpkan2025}，以寻求进一步的复杂度降低；（d）评估MLP-NLC与概率星座整形技术的联合使用效果，后者恰好在非线性补偿收益最大的高功率区间发挥作用。
